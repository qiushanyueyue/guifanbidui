...
- 生产与数据快照：见 [`docs/project-memory/CURRENT.md`](docs/project-memory/CURRENT.md)，快照日期 `2026-09-06`。
- 当前生产功能提交为 `ef323fe`；现有 Vercel 项目与 `guifan.108923.xyz` 已部署并验收。
- 2026-09-06 生产复核：1738 条；374 `current`、28 `abolished`、1336 `unknown`、0 `conflict`；来源健康为官方三源 `never`、搜建筑 `never`、工标网 `partial`。
- 最近验证基线：后端 127 项、前端 14 项、ESLint、Vite build 通过；公网真实页面验收 `GB 50009-2012`、`GB 50010-2010`、`GB 50011-2010` 未找到为 0，同规范并发查询均成功。
...
- 生产仍有 1336 条 `unknown`；漏收补查、单源明确结论及“完全一致”已部署，正常规范明确判断率 ≥95% 仍需继续批量验收。
