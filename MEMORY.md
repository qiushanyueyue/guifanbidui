...
- 2026-09-01 公网只读复核：1740 条；364 `current`、27 `abolished`、1349 `unknown`、0 `conflict`；来源健康为官方三源 `never`、搜建筑 `never`、工标网 `partial`。
- 最近本地验证基线：后端 120 项、前端 14 项、黄金案例 50 项通过；浏览器实测漏收补查、编号属性修正、“完全一致/现行”和搜建筑全文链接，控制台无错误。
...
- 生产仍有 1349 条 `unknown`；本地漏收补查、单源明确结论及“完全一致”新策略尚未部署，正常规范明确判断率 ≥95% 仍是未验收目标。
